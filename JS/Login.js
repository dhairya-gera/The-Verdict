    ...
    function sendOTP(email, name) {
        ...
        console.log('%c[THE VERDICT] OTP for ' + email + ': ' + generatedOTP, 'background:#3AADFC; color:#030a12; font-size:14px; font-weight:bold; padding:6px 12px; border-radius:4px;');
        ...
        document.getElementById('step-1').querySelector('.step-circle').innerHTML = '<i class="fa-solid fa-check" style="font-size:0.7rem"></i>';
        ...
        startCountdown();
        showToast('info', '<i class="fa-solid fa-envelope"></i>', 'OTP generated! Check browser console (F12).');
    }
    ...
    function verifyOTP() {
        let entered = '';
        for (let i = 0; i < 6; i++) entered += document.getElementById('otp' + i).value;
 
        if (entered.length < 6) {
            showAlert('otp-error', 'otp-error-msg', 'Please enter the complete 6-digit OTP.');
            return;
        }
 
        const btn = document.querySelector('#reg-step-2 .submit-btn');
        btn.classList.add('loading');
        ...
        setTimeout(() => {
            ...
            if (entered === generatedOTP) {
                ...
                document.getElementById('step-2').querySelector('.step-circle').innerHTML = '<i class="fa-solid fa-check" style="font-size:0.7rem"></i>';
                document.getElementById('step-3').classList.add('active');
                document.getElementById('reg-step-2').style.display = 'none';
                document.getElementById('reg-step-3').style.display = 'block';
                document.getElementById('welcome-name').textContent = name;
                showToast('success', '<i class="fa-solid fa-circle-check"></i>', 'Account created successfully!');
            } else {
                showAlert('otp-error', 'otp-error-msg', 'Incorrect OTP. Please try again.');
                // Shake all OTP boxes
    ...
    function resendOTP() {
        ...
        clearAlerts();
        generatedOTP = Math.floor(100000 + Math.random() * 900000).toString();
        console.log('%c[THE VERDICT] New OTP for ' + email + ': ' + generatedOTP, 'background:#f0b429; color:#030a12; font-size:14px; font-weight:bold; padding:6px 12px; border-radius:4px;');
        startCountdown();
        showToast('info', '<i class="fa-solid fa-rotate-right"></i>', 'New OTP sent! Check console (F12).');
    }
